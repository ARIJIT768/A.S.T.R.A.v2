  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (session && isMounted) {
        setAccountName(session.user?.user_metadata?.display_name || "Doctor");
      }
      await fetchHistory();
    };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <header className="flex justify-between items-center border-b border-slate-800 pb-6">
          ...
          <div className="flex gap-4 items-center">
            ...
            <button onClick={fetchHistory} className={`p-2 rounded-lg bg-slate-900 border border-slate-800 hover:border-cyan-500 transition-all ${isRefreshing ? 'animate-spin' : ''}`}>
              🔄
            </button>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[500px]">
          ...
          <div className="lg:col-span-2 flex flex-col bg-slate-900/50 rounded-3xl border border-slate-800 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth">
               {history.length > 0 ? (
                 history.slice(0, 5).reverse().map((msg, i) => (
